    ...
    function calculatePrices() {
        ...
        receiptItems.forEach(item => {
            ...
            if (item.userTags && item.userTags.length > 1) {
                ...
                const remainderTimes100 = Math.trunc((parseFloat(item.price) - roundedPrice*item.userTags.length)*100);
                console.log("Rounded Price: " + roundedPrice, "Item name: " + item.name, "Participant ID: " + participantId, "Remainder: " + remainderTimes100);
                if(participantId > item.userTags.length-remainderTimes100) {
                    roundedPrice += 0.01;
                }

                const currentDiscount = (item.discount ? parseFloat(item.discount) : 0) * currentPercentage * .01;
